...
| 功能 | 說明 |
|---|---|
| 新增退款 | 秘書搜尋客戶、選擇租約，填寫退款原因與金額明細後建立退款單；客戶搜尋結果與租約下拉皆會排除已解約的客戶/合約（見第 7.2 節） |
...
- **中途解約押金上限**：`mid_term_termination` 這個判斷完全依賴秘書在新增/修改退款時手動勾選，系統不會自動偵測，也不會反向影響租約狀態（計算押金上限時，租約既有的 `lease_status`／`termination_date_text` 不影響此計算結果；這個勾選框現在**只**用來決定押金上限，不再觸發任何租約回壓，見第 4.1a 節）。勾選後，若該租約月租金低於押金，退款基準會自動改成月租金；已收取之預收租金本來就不在 `refund_amount` 計算範圍內（公式從頭到尾不讀 `rent_payments`），不需要另外處理。
...
| PUT | `/api/refunds/{id}` | 修改退款（依狀態限制可改欄位，見第 3 節；「審核通過」→「已退款」時**不會**動到租約的 `lease_status`/`termination_date_text`，見第 4.1a 節） | 無限制 |
...
2. 選擇該客戶的租約（下拉會顯示辦公室編號/登記＋押金金額）。此下拉只列出該客戶名下 `lease_status` 不是「已解約」的合約（`activeContracts()`），已解約的合約不會出現；若客戶同時有多筆未解約合約，會列出多個選項供分辨。
...
- **「中途解約」完全依賴秘書手動勾選，系統不會自動判斷**：`mid_term_termination` 只由退款表單的勾選框決定，不會比對租約的到期日或解約日期，勾錯不會有任何系統警示或驗證，需秘書自行確認。這個勾選框目前只影響押金上限計算，不會再連動租約狀態（見第 4.1a 節）。
