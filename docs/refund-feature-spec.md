...
| 中途解約押金上限 | 新增/修改退款時秘書勾選「中途解約」，可退押金上限自動改為一個月租金；未勾選（合約到期解約）維持全額押金 |
| 辦理退款自動回壓租約 | 退款完成（審核通過 → 已退款）時，系統自動將對應租約的 `lease_status` 寫回「已解約」，不需秘書另外去租約管理手動修改 |
...
- 退款完成（狀態轉為「已退款」）時，系統也會**自動把對應租約 `contracts.lease_status` 寫回「已解約」**，不論是否勾選中途解約，皆視為租約已終止（見第 4.1 節）。
...
```
押金基準 = 若勾選「中途解約」且 contracts.rent < contracts.deposit → contracts.rent（一個月租金）
           否則 → contracts.deposit（全額押金，含未勾選「中途解約」＝合約到期解約的情況）

refund_amount = 押金基準 + adjustment_amount − deduction_total
```

- **中途解約押金上限**：`mid_term_termination` 這個判斷完全依賴秘書在新增/修改退款時手動勾選，系統不會自動偵測（租約的 `lease_status`／`termination_date_text` 不影響這個計算，見第 8 節已知限制）。勾選後，若該租約月租金低於押金，退款基準會自動改成月租金；已收取之預收租金本來就不在 `refund_amount` 計算範圍內（公式從頭到尾不讀 `rent_payments`），不需要另外處理。
  - API 回應會附帶 `message` 提示文字（例如：「中途解約，退款金額已依規定以一個月租金（NT$17000）為押金退還上限。」），前端直接顯示於畫面上方提示列。
- 若計算結果為負數：`refund_amount` 存為 `0`（不允許負數退款），並依第 2.2 節規則在 `charge_lists` 建立（或沿用既有）一筆未結清帳單。
  - API 回應會附帶 `message` 提示文字（例如：「扣款總額大於應退金額，差額 NT$X 已自動建立收費清單（未結清，編號 #Y），請至收費清單管理確認金額並收款。」），前端直接顯示於畫面上方提示列。中途解約與超額扣款兩則提示可能同時出現。
...
| POST | `/api/refunds` | 新增退款（body 含 `midTermTermination`，見第 4.1 節） | 無限制 |
| PUT | `/api/refunds/{id}` | 修改退款（依狀態限制可改欄位，見第 3 節；「審核通過」→「已退款」時會自動回壓租約 `lease_status`，見第 3 節） | 無限制 |
...
- 點擊任一列會開啟「退款詳細資料」彈窗，顯示完整欄位（租約編號、調整金額/備註、扣款總額、解約類型、建立人/時間、審核人/時間、退款方式、收款帳戶、退款日期），並依目前狀態顯示對應操作按鈕（修改／送交審核／審核通過／辦理退款／取消）。「送交審核」僅在狀態為「草稿」時出現，用來把草稿轉為「待審核」交給主管審核。

### 7.2 新增退款
1. 輸入公司名稱搜尋客戶並選擇。
2. 選擇該客戶的租約（下拉會顯示押金金額）。
3. （選填）輸入收費清單編號並按「帶入扣款金額」，自動預填扣款總額。
4. 填寫退款原因（必填）、調整金額／備註、扣款總額、勾選「中途解約」（勾選＝中途解約、可退押金上限一個月租金；不勾＝合約到期解約、退全額押金）、退款方式與收款帳戶。
5. 選擇送出後狀態（「草稿」或「待審核」）並送出。
6. 若觸發中途解約押金上限或超額扣款，畫面上方會顯示提示訊息（超額扣款會附自動建立的收費清單編號），需另外到「收費清單管理」畫面確認/收款。

### 7.3 修改／辦理退款
- 「草稿」「待審核」狀態：詳細彈窗按「修改」開啟編輯面板，可改動所有欄位（含「中途解約」勾選框）。
- 「審核通過」狀態：按「辦理退款」開啟同一編輯面板，僅能填寫付款資訊與退款日期，金額欄位與「中途解約」勾選框鎖定為唯讀。送出後狀態轉為「已退款」，系統會自動把對應租約的 `lease_status` 寫回「已解約」（見第 3 節）。
...
- **押金金額為即時讀取，非鎖定快照**：計算退款金額時即時查詢 `contracts.deposit`（中途解約時另查 `contracts.rent`），若之後押金或租金被更改，既有退款單的金額不會自動重算，需手動修改。
- **「中途解約」完全依賴秘書手動勾選，系統不會自動判斷**：`mid_term_termination` 只由退款表單的勾選框決定，不會比對租約的到期日或解約日期，勾錯不會有任何系統警示或驗證，需秘書自行確認。
- **辦理退款一律回壓租約為「已解約」，不分是否勾選中途解約**：只要退款走到「已退款」，系統就會把對應 `contracts.lease_status` 改成「已解約」，目前沒有例外情境（例如退款原因不是租約終止）可以跳過這個回壓。
